        ...
        foreach(Player p in players)
        {
          try
          {
            ...
            if (distanceFromPlayer < 12)
            {
              ...
              projectiles.Add(new Projectile(x - 0.5, y - 0.6, angleTowardsPlayer, -1) { type = 1 });
              projectiles.Add(new Projectile(x - 0.5, y - 0.6, angleTowardsPlayer + Math.PI / 8, -1) { type = 1 });
              projectiles.Add(new Projectile(x - 0.5, y - 0.6, angleTowardsPlayer - Math.PI / 8, -1) { type = 1 });
